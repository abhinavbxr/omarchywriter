...
This release removes the right-click context-menu GUI, applies the new simple
editor and window defaults, adds the `show_tab_bar` configuration option, and
turns autosave on by default. Existing configuration files are upgraded on
startup only where they still contain old generated defaults; custom values
are preserved.
...
The installer updates the private virtual environment from the checkout you
run it from. Pull the new release first, then rerun the installer:
...
The application migrates old generated config values on its next start and
does not overwrite personal configuration choices.
...
All uncommented options are active. `Ctrl+O` opens a document and `Ctrl+S`
saves it. To change a setting, edit `config.toml` in another editor, save it,
and watch the running OmarchyWriter window update.

The editor does not show a context menu on right-click. Use the usual keyboard
...
The tests cover configuration parsing, safe legacy-config migration, Omarchy color mapping without writing
...
configuration recovery. The latest verification completed with 11 tests
